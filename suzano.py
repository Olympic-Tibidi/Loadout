...
if select=="LOADOUT" :
    ...
    if len(dispatched.keys())>0 and not no_dispatch:
        ...
        if st.button('SUBMIT EDI'):
            ...
            bill_of_ladings[bill_of_lading_number]:{"vessel":vessel,"release_order":release_order_number,"sales_order":sales_order_item,"carrier_id":carrier_code,"vehicle":vehicle_id,"quantity":st.session_state.updated_quantity}            st.write(bill_of_ladings)
