...
if authentication_status:
    ...
    if username == 'ayilmaz' or username=='gatehouse':
        ...
        select=st.sidebar.radio("SELECT FUNCTION",
            ('ADMIN', 'LOADOUT', 'INVENTORY','DATA BACKUP','FINANCE'))
        ...
        if select=="LOADOUT" :
            ...
            if len(dispatched.keys())>0 and not no_dispatch:
                ...
                st.write(load_admin_bill_of_ladings.index[-2:])
                if yes:
        ...
        if select=="INVENTORY" :
            ...
            with inv1:
                ...
                with daily1:
                    ...
                    st.markdown(f"<p style='font-family:fantasy,monospace;'>{text}</p>",unsafe_allow_html=True)     
